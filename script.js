function main() {
    ...
    const store = {
        ...
        state: {
            ...
            config: {
                quality: String(
                    IS_HIGH_END_DEVICE ? QUALITY_HIGH : QUALITY_NORMAL
    ...
    const helpContent = {
        shellType: {
            header: "Loại Pháo",
            body: 'Loại pháo hoa sẽ được tung ra. Chọn "Random" để có một loại tốt đẹp!',
        },
        shellSize: {
            header: "Kích Cỡ",
        ...
        finaleMode: {
            header: "Finale Mode",
            body: 'Tung ra những đợt pháo hoa dữ dội. Có thể gây lag. Yêu cầu bật "Tự Động".',
        },
        hideControls: {
    ...
    const crysanthemumShell = (size = 1) => {
        ...
        const secondColor =
            singleColor && (Math.random() < 0.2 || color === COLOR.White)
                ? pistilColor ||
                  randomColor({ notColor: color, limitWhite: true })
                : null;
        const streamers =
    ...
    function init() {
        ...
        setOptionsForSelect(appNodes.skyLighting, [
            ...
            { label: "Thường", value: SKY_LIGHT_NORMAL },
        ]);
    ...
    function seqPyramid() {
        ...
        function launchShell(x, useSpecial) {
            ...
            const shell = new Shell(
                shellType(useSpecial ? largeSize : smallSize)
            );
    ...
    function colorSky(speed) {
        ...
        targetSkyColor.r =
            (targetSkyColor.r / maxColorComponent) *
            maxSkySaturation *
            intensity;
        targetSkyColor.g =
            (targetSkyColor.g / maxColorComponent) *
            maxSkySaturation *
            intensity;
        targetSkyColor.b =
            (targetSkyColor.b / maxColorComponent) *
            maxSkySaturation *
            intensity;
    ...
    const audio = document.querySelector(".music-controls audio");
    const btnMusic = document.querySelector(".music-controls .icon");
    const songs = document.querySelector(".music-controls .songs");
    let currentIndex = 0;
    ...
    audio.onplay = function () {
        musicPlaying = true;
        btnMusic.innerHTML = `<i class='bx bx-pause'></i>`;
    };

    audio.onpause = function () {
        musicPlaying = false;
        btnMusic.innerHTML = `<i class='bx bx-play'></i>`;
    };

    audio.onended = function () {
        currentIndex++;
        if (currentIndex == LIST_SONG.length) currentIndex = 0;
        songs.value = String(currentIndex);
        audio.src = `./music/${LIST_SONG[currentIndex].slug}.mp3`;
        audio.load();
        audio.play();
    };
    ...
    songs.onchange = function () {
        currentIndex = Number(songs.value);
        audio.src = `./music/${LIST_SONG[currentIndex].slug}.mp3`;
        audio.load();
        audio.play();
    };
...
(function () {
    ...
    const countDown = new Date(birthday).getTime(),
        x = setInterval(function () {
            ...
            if (distance < 0) {
                document.querySelector(".countdown-timer").style.display =
                    "none";
                main();
                clearInterval(x);
            }
...
previewCode.onchange = function () {
    if (this.value.trim() == "ngocanhxinhgai") {
        ...
        previewCode.style.borderColor = "#fa6161";
        previewCode.value = "";
        previewCode.placeholder = "Mã xem trước không đúng";
    }
};
